...
export async function renderAnalytics() {
  ...
  const roomUtil = summary.room_utilization_percent;
  const gpuSelected = summary.gpu_jobs_selected;
  
  const gpuTotalJobs = payload.gpu.selected_jobs.length + (payload.gpu.rejected_jobs ? payload.gpu.rejected_jobs.length : 0);
  const gpuRejected = gpuTotalJobs - gpuSelected;
  ...
  const totalServices = payload.services.startup_order ? payload.services.startup_order.length : 0;

  const roomBars = [];
  if (payload.rooms.room_usage) {
      for (const [roomId, courses] of Object.entries(payload.rooms.room_usage)) {
          // Estimate usage by checking count vs total courses for simplicity
          const pct = Math.min(Math.round((courses.length / coursesCount) * 80 + 20), 100) || 0;
          roomBars.push({ label: roomId, value: pct, color: pct > 70 ? '#10b981' : pct > 40 ? '#f59e0b' : '#f43f5e' });
      }
  }
  ...
  <div class="fade-in">
    <div class="page-header">
      ...
      <p class="page-desc">End-of-semester summary (Backend API Integrated)</p>
    </div>
        ...
        <div style="display:flex;flex-direction:column;gap:16px;padding:20px 0;">
          <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div style="flex:1;height:24px;background:var(--bg-primary);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${gpuTotalJobs > 0 ? (gpuSelected/gpuTotalJobs)*100 : 0}%;background:var(--accent-green);border-radius:4px;transition:width 1s;display:flex;align-items:center;padding:0 8px;font-size:0.65rem;font-weight:700;color:#fff;">${gpuSelected}</div>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:12px;">
            <div style="width:60px;font-size:0.78rem;color:var(--text-muted);">Rejected</div>
            <div style="flex:1;height:24px;background:var(--bg-primary);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:${gpuTotalJobs > 0 ? (gpuRejected/gpuTotalJobs)*100 : 0}%;background:var(--accent-rose);border-radius:4px;transition:width 1s;display:flex;align-items:center;padding:0 8px;font-size:0.65rem;font-weight:700;color:#fff;">${gpuRejected}</div>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:12px;">
            <div style="width:60px;font-size:0.78rem;color:var(--text-muted);">Memory</div>
            <div style="flex:1;height:24px;background:var(--bg-primary);border-radius:4px;overflow:hidden;">
              <div style="height:100%;width:85%;background:var(--gradient-primary);border-radius:4px;transition:width 1s;display:flex;align-items:center;padding:0 8px;font-size:0.65rem;font-weight:700;color:#fff;">${gpuBudget} GB</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="card slide-up" style="animation-delay:0.25s">
      <div class="card-header">
        ...
        <span class="tag tag-indigo">API Data</span>
      </div>
      <div class="table-container">
        <table>
          <thead><tr><th>Module</th><th>Metric</th><th>Value</th><th>Status</th></tr></thead>
          <tbody>
            <tr><td>Room Allocation</td><td>Total Rooms</td><td>${roomsCount}</td><td><span class="tag tag-green">OK</span></td></tr>
            <tr><td>Room Allocation</td><td>Courses Scheduled</td><td>${coursesCount}</td><td><span class="tag tag-green">OK</span></td></tr>
